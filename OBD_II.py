...
try:
    ...
    user_uid = os.getenv('DRIVER_UID', 'Unknown')
    ...
    outfile_path = f'{user_uid}_{current_time}.csv'
    ...
    try:
        ...
        with open(outfile_path, 'w', newline='') as csvfile:
            fieldnames = ['timestamp', 'datetime', 'count', 'temperature', 'rpm', 'speed', 'throttle', 'fuel',
                          'speedLimit',
                          'acceleration']
        ...
        def upload_file_to_storage(file_path, collection_name, document_id):
            blob = bucket.blob(f'drives/{user_uid}/{os.path.basename(file_path)}')
        ...
        document_id = f'{user_uid}_{int(time.time())}'
    ...
    print(f'See you again{user_uid}')
except Exception as e:
    print(f"Error: {e}")
    obd_device.updateStatus(str(e))
    obd_device.updateError("Error")
    sys.exit()
